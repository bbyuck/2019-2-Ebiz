...
<%@ page import="java.security.*, kr.co.youiwe.webservice.*" %>
<%!
...
<jsp:useBean id="smsid" scope="session" class="kr.co.youiwe.webservice.ServiceSMSSoapProxy" />
<%
	...
	try {
		...
		case 2:							//즉시보내기 사용예제
			...
			String sendSMS = smsid.sendSMS(smsID, encrypt.getEncryptData(), senderPhone, receivePhone, smsContent);
		 	...
		 	response.sendRedirect("../do/exchange_do.jsp");
		...
		case 90:								// 주간 발송량/제한량 확인시 사용
			gotMethod = true;
			smsID =  request.getParameter("smsID");
			hashValue =  request.getParameter("hashValue");
			encrypt = new CEncrypt("MD5", smsID + hashValue);
			String getWeeklyLimit = smsid.getWeeklyLimit(smsID, encrypt.getEncryptData());
			String[] res = getWeeklyLimit.split("\\|");
			out.println("주간총 제한량 : "+ res[0] + " 주간 총 발송량 : " + res[2] + "\n주간 URL 제한량 : " + res[1] + " 주간 URL 발송량 : " + res[3]);
			break;
